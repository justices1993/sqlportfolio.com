...
(
  ...
  GROUP BY city)
, percentage_spends AS
(
   SELECT SUM(CAST(amount as bigint)) as amount_spent
   FROM credit_card_transcations
)
SELECT TOP 5 city
,total_amount as amount_spent
,ROUND(total_amount*1.0/amount_spent*100,2) as percentage_contribution
FROM CTE 
INNER JOIN percentage_spends 
ON 1=1
...
SELECT a.card_type, a.yt, a.mt, a.total_amount
FROM (SELECT card_type, yt, mt, total_amount
